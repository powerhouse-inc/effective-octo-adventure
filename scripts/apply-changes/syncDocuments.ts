import { AtlasFoundationClient } from "./AtlasFoundationClient";
import { AtlasScopeClient } from "./AtlasScopeClient";
...
async function syncDocuments(config: DocumentSyncConfig) {
    ...
    if (driveIds.includes(config.driveName)) {
      ...
      const systemClient = new SystemGraphClient(new URL("/system", config.gqlEndpoint).href);
      console.log(`Creating drive ${config.driveName}...`); 
      const newDriveResult = await systemClient.createDrive(config.driveName, config.preferredEditor, config.driveName, config.driveName);
