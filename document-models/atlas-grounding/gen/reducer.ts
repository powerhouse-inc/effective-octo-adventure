...
import { type AtlasGroundingDocument, z } from "./types.js";
...
const stateReducer: StateReducer<AtlasGroundingDocument> = (
  state,
  action,
  dispatch,
) => {
  ...
  switch (action.type) {
    case "SET_GROUNDING_NAME":
      z.SetGroundingNameInputSchema().parse(action.input);
      GeneralReducer.setGroundingNameOperation(
        state[action.scope],
        action,
        dispatch,
      );
...
export const reducer = createReducer<AtlasGroundingDocument>(stateReducer);
