...
export const getResolvers = (subgraph: ISubgraph): Record<string, any> => {
  ...
  return {
        ...
        return {
          getDocument: async (args: any) => {
            ...
            const doc = await reactor.getDocument<
              PHDocument<AtlasFoundationPHState>
            >(driveId, docId);
            return {
              driveId: driveId,
              ...doc,
          ...
          getDocuments: async (args: any) => {
            ...
            return docs.filter(
              (doc) => doc.header.documentType === "sky/atlas-foundation",
            );
          },
        };
      },
    ...
    Mutation: {
      ...
      AtlasFoundation_replaceContextData: async (_: any, args: any) => {
        const driveId: string = args.driveId || DEFAULT_DRIVE_ID;
        const docId: string = args.docId || "";
        const doc = await reactor.getDocument(driveId, docId);
